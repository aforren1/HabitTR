function presponse = getResponseProbs_1process(RT,paramsA,paramsB,initAE)
...
alpha(1,:) = [initAE initAE qB qB];

% coefficients for probability of acting according to mapping A (habit)
alpha(2,:) = [initAE initAE (1-qB)/3 (1-qB)/3];

% coefficients for other keys
alpha(3,:) = [.5-initAE .5-initAE (1-qB)/3 (1-qB)/3];
