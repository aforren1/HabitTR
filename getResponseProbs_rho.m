function presponse = getResponseProbs_rho(RT,paramsA,paramsB,initAE,rho)
...
alpha(1,:) = [initAE rho*(1-qA)/3+(1-rho)*initAE qB qB];

% coefficients for probability of acting according to mapping A (habit)
alpha(2,:) = [initAE rho*qA+(1-rho)*initAE (1-qB)/3 (1-qB)/3];

% coefficients for other keys
alpha(3,:) = [.5-initAE rho*(1-qA)/3+(1-rho)*(.5-initAE) (1-qB)/3 (1-qB)/3];
